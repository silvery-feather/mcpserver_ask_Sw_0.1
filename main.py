...
变更要点：
- ask(question, prefer_docs=True) 现在只做向量检索，返回 JSON：
  {
    "query": 原始问题,
...
- 服务端不再调用大模型 chat；生成回答交由 Cherry 处理。
- 继续保留 index_docs、--index/--auto 等建索引流程。

用法（与原版一致）：
1) 安装依赖：
   uv add mcp openai faiss-cpu numpy regex tqdm
2) 配置下方 CONFIG。fass
3) 首次建索引：
   uv run python main_single_tool_swanlab_retriever_only.py --index "D:/mcpserver/md"
4) 启动（stdio）：
   uv run python main_single_tool_swanlab_retriever_only.py
5) 客户端调用示例：
   ask(question="如何在 SwanLab 中创建 API Token？")
   → 返回 JSON（仅检索片段），由 Cherry 进一步生成答案与引用。
"""
...
# CONFIG（写死配置）
# =====================
INDEX_DIR = r"D:\mcpserver\rag_cache\api_v1"         # 索引目录
DEFAULT_DOCS_ROOT = r"D:\mcpserver\md"                 # 文档目录（仅 --auto 使用）
API_KEY = ""          # ← 填你的 DashScope Key（建议改为环境变量）
BASE_URL = "https://dashscope.aliyuncs.com/compatible-mode/v1"
EMBED_MODEL = "text-embedding-v2"

CHAT_MODEL = "qwen-plus"   # 新：用于简化用户长输入的对话模型（Qwen-Plus）
EMBED_MODEL = "text-embedding-v2"

# RAG / chunk 参数
TEMPERATURE = 0.2           # 仅保留以保持兼容；未使用于生成
CHUNK_SIZE = 900
CHUNK_OVERLAP = 120
MAX_INPUT_CHARS = 12000
SEARCH_K = 8
ANSWER_K = 6               # 作为返回的 hits 上限
MAX_EMBED_BATCH = 25       # DashScope embeddings 单次最多 25
RAG_SCORE_THRESHOLD = 0.50 # 命中阈值（余弦相似度）
